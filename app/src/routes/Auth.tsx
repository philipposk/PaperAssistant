...
import { sendMagicLink, signInWithGitHub, signInWithGoogle, useAuth } from "../lib/auth";

export function Auth() {
  const { isCloudConfigured, isSignedIn } = useAuth();
  const [email, setEmail] = useState("");
  const [sent, setSent] = useState(false);
  const [busy, setBusy] = useState<"" | "magic" | "google" | "github">("");
          ...
          {sent ? (
            ...
            <form onSubmit={magic} className="rounded-[var(--radius-lg)] border border-[var(--color-line)] bg-[var(--color-surface)] p-5">
              ...
              <button
                ...
                className="mt-2 w-full px-4 py-2 rounded-md border border-[var(--color-line)] hover:bg-[var(--color-surface-2)] text-sm flex items-center justify-center gap-2 disabled:opacity-60"
              >
                {busy === "github" ? (
                  <Loader2 size={14} className="animate-spin" />
                ) : (
