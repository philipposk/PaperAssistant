// stripe-checkout — start a Stripe Checkout session to upgrade to the Plus tier.
// Auth: Supabase JWT. Returns { url } to redirect the browser to Stripe.
...
Deno.serve(async (req) => {
  ...
  // reuse an existing Stripe customer for this user+app if we have one
  ...
  if (!customer) {
    const c = await stripe.customers.create({
      ...
      metadata: { supabase_user_id: user.id, app: APP },
    });
    customer = c.id;
  }
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${APP_URL}/settings?sub=success`,
    cancel_url: `${APP_URL}/settings?sub=cancel`,
    metadata: { supabase_user_id: user.id, app: APP, plan: "plus" },
    subscription_data: { metadata: { supabase_user_id: user.id, app: APP, plan: "plus" } },
  });
